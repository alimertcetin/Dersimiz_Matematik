...
public class KeycardEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        Keycard_Script _keycard = (Keycard_Script)target;

        if (GUI.changed)
        {
            if (_keycard.Keycard == Door_and_Keycard_Level.Yesil)
            {
                Debug.Log("Seviye 1 seçildi.");
            }
            if (_keycard.Keycard == Door_and_Keycard_Level.Sari)
            {
                Debug.Log("Seviye 2 seçildi.");
            }
            if (_keycard.Keycard == Door_and_Keycard_Level.Kirmizi)
            {
